...
База — **CARTO Positron** + тонировка под сепию:

```css
.leaflet-tile-pane { filter: sepia(.22) saturate(.85) hue-rotate(-8deg) contrast(.95); }
```

Фильтр **только на `.leaflet-tile-pane`**, не на `.leaflet-container`: иначе под него попадут пины, попапы и контролы.

Атрибуцию CARTO проверить и поставить рядом с атрибуцией OSM. Stamen Watercolor (через Stadia) отлично попадает в концепцию, но топонимы читаются плохо — держим как возможный переключатель «акварельный режим» на потом, не как базу.
